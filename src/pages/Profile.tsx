...
const Profile = () => {
  ...
  // Fetch latest handly_users data to populate additional fields
  useEffect(() => {
    const fetchHandlyData = async () => {
      if (!user) return;
      ...
      try {
        ...
        console.error('Error fetching handly data:', error);
      }
    };
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    try {
      // Update profiles table - use update instead of upsert to avoid unique constraint issues
      ...
      if (profileError) {
        toast({
          title: "Error",
          description: "Unable to update profile: " + profileError.message,
          variant: "destructive"
        });
        return;
      }
